...
class Tasks(unittest.TestCase):
    ...
    def test_update_task(self):
        ...
        # Create a task in a section
        section_id = self.section_id
        created_task_id = self.create_task(section_id=section_id).json()["id"]
        LOGGER.info("Task Id: %s", created_task_id)

        # Update the created task
        data_for_update = {
            "content": "Task updated",
            "due_string": "next friday at 10:00"
        }
        url_task_id = f"{self.url_tasks}/{created_task_id}"
        response_update = RestClient().send_request("post", session=self.session, headers=HEADERS,
                                                    url=url_task_id, data=data_for_update)
    ...
    def test_delete_task(self):
        ...
        url_task_id = f"{self.url_tasks}/{created_task_id}"
        response_update = RestClient().send_request("delete", session=self.session, headers=HEADERS,
                                                    url=url_task_id)
